...
object SpecToShEx extends App with LazyLogging {
  ...
  val shexFile = new File(args(1))
  val shexShapeMapFile = new File(args(2))
  if (shexFile.exists && !shexFile.canWrite) throw new RuntimeException(s"Output file ${shexFile} is not writeable.")
  if (shexShapeMapFile.exists && !shexShapeMapFile.canWrite) throw new RuntimeException(s"ShEx shape file ${shexShapeMapFile} is not writeable.")
  ...
  val shexShapeMap = new PrintWriter(new FileWriter(shexShapeMapFile))

  val output = new PrintWriter(new FileWriter(shexFile))
  val prefixes = s"""PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
  ...
  shexShapeMap.println(prefixes)

  val cardinalityRegex = """^(\d+)..(\d+|\*)$""".r
  var shapeMap = Seq[String]()
  ...
  val allAttributes = entitiesById.keys.toSeq.sorted.foreach(entityId => {
    ...
    entitiesById(entityId).flatMap(entity => {
      ...
      if (entity.contains("iri") && !entity("iri").isEmpty) {
        shapeMap = shapeMap ++ Seq(s"{FOCUS rdf:type ${entity("iri")}}@cgshapes:${entity("name")}")
      }
  ...
  // Write out shape map.
  shexShapeMap.println(shapeMap.mkString(",\n"))
